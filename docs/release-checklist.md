...
4. 检查生产环境变量。EC2 上的文件固定在：
   ...
   必需项包括 `NEXT_PUBLIC_APP_URL`、`NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_ANON_KEY`、`SUPABASE_SERVICE_ROLE_KEY`、`LINGYA_API_KEY`、`PLATO_API_KEY`、`JOB_PROCESSOR_SECRET`。启用 OSS、Stripe 或其他供应商时，同步检查对应密钥。
...
工作流会把 `PLATO_BASE_URL`、`PLATO_API_KEY`、`PLATO_GPT_IMAGE_MODEL` 写入 EC2 的共享 `.env.production`。其余生产变量仍由 EC2 上的共享环境文件负责。
