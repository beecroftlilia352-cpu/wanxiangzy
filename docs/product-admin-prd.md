...
| P1 项 | 当前落地 | 说明 |
| --- | --- | --- |
| Prompt 模板 A/B 实验 | 已落地配置管理 V1 | `/admin/prompts` 基于 `admin_config_versions` 管理 `prompt.experiments`，支持实验分流、变体权重、指标门禁、发布/归档回滚、`/api/admin/prompts`、专用写 API、保存视图和 CSV 导出数据集。运行时代码消费、逐字段 prompt diff、低成本试运行和自动门禁阻断属于后续增强。 |
