                ...
                <details className="mt-3 rounded-md border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] px-3 py-2">
                  ...
                  <div className="mt-3 rounded-md border border-[var(--admin-border)] bg-[var(--admin-surface)] p-3">
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-3">
                      ...
                      {deployment.protocol === "kie-job" && <CompactField label="图生图上游模型（带参考图时）"><input className={compactInput} value={deployment.adapterConfig?.editUpstreamModel || ""} onChange={(event) => update(index, { adapterConfig: { ...(deployment.adapterConfig || {}), editUpstreamModel: event.target.value || undefined } })} placeholder="gpt-image-2-image-to-image" /></CompactField>}
                      {deployment.protocol === "kie-job" && <CompactField label="输入图字段名（默认 image_input）"><input className={compactInput} value={deployment.adapterConfig?.imageInputField || ""} onChange={(event) => update(index, { adapterConfig: { ...(deployment.adapterConfig || {}), imageInputField: event.target.value || undefined } })} placeholder="image_input / image_urls / input_urls" /></CompactField>}
                      {deployment.protocol === "kie-job" && <CompactField label="尺寸字段名（默认 resolution，none=不发送）"><input className={compactInput} value={deployment.adapterConfig?.imageSizeField || ""} onChange={(event) => update(index, { adapterConfig: { ...(deployment.adapterConfig || {}), imageSizeField: event.target.value || undefined } })} placeholder="resolution / none" /></CompactField>}
                    </div>
