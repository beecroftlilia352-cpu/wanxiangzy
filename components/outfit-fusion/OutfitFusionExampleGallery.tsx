...
export function OutfitFusionExampleGallery({
  ...
  return (
    <section className={cn("mx-auto w-full max-w-[1680px]", className)} aria-label="搭配融图示例">
      <div className="grid overflow-hidden rounded-[8px] bg-white grid-cols-1 gap-[3px] sm:grid-cols-[repeat(auto-fill,minmax(240px,1fr))] xl:grid-cols-[repeat(auto-fill,minmax(260px,1fr))]">
        {templates.map((template, index) => (
          <article
            key={template.id}
            aria-label={template.title}
            className={cn(
              "group relative aspect-[3/4] min-h-[320px] animate-slide-up overflow-hidden bg-slate-100 shadow-sm transition-[transform,box-shadow,filter] duration-[420ms] ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-0.5 hover:shadow-[0_18px_46px_rgba(15,23,42,0.14)] motion-reduce:animate-none sm:min-h-[360px]",
