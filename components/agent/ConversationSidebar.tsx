    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col border-r border-slate-200/80 bg-white/95 backdrop-blur-xl transition-transform duration-200 lg:static lg:translate-x-0 ${
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {sorted.length === 0 ? (
            <div className="px-4 py-8 text-center text-xs text-slate-400">暂无对话</div>
          ) : sorted.map((conv) => {
            const isActive = conv.id === activeId;
            return (
              <div key={conv.id}
                className={`group flex cursor-pointer items-center gap-2.5 mx-1 rounded-xl px-3 py-2.5 transition-colors ${
                ...
                <div className="min-w-0 flex-1">
                  <p className={`truncate text-sm font-medium ${isActive ? "text-violet-800" : ""}`}>{conv.title || "新对话"}</p>
                  <p className="text-[11px] text-slate-400">{formatTime(conv.updated_at)}</p>
                </div>
