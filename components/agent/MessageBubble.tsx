...
import { Bot, User, Loader2, CheckCircle2, AlertCircle, Download, ZoomIn, RefreshCw, Copy, Clock, Sparkles } from "lucide-react";
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {images.map((img, i) => (
              <button key={i} onClick={() => onOpenImage(img.url)}
                className="group relative h-14 w-14 overflow-hidden rounded-lg border border-violet-200 shadow-sm">
          ...
          <div className="mt-1.5 w-full max-w-md">
            ...
            <div className="mb-2 flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2">
              ...
              {generation.creditsUsed ? (
                <span className="text-[11px] text-emerald-600">消耗 {generation.creditsUsed} 积分</span>
              ) : null}
              {generation.module && (
                <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-[10px] font-medium text-emerald-600">{generation.module}</span>
              )}
            </div>
            {/* 结果图片 */}
            ...
            <div className="px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-red-500">{generation.error || "未知错误"}</p>
                </div>
              </div>
              <button onClick={() => onRetry(message.id)}
