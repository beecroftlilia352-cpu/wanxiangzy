      ...
      >
          <DialogHeader className="flex-row items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 pr-14 text-left">
            <div className="min-w-0">
              ...
              <DialogDescription className="mt-1 text-xs text-slate-500">从已完成作品中复用图片作为服装输入。</DialogDescription>
            </div>
                ...
                {items.map((item) => (
                  ...
                  >
                    <div className="studio-checkerboard aspect-[4/5] overflow-hidden">
                      <RawPreviewImage src={getImageVariantUrl(item.url, "card")} alt={`作品库图片：${item.label}`} width={320} height={400} loading="lazy" decoding="async" className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
                    </div>
