...
export function Column({ data = [], xField = "label", yField = "value", height = 280, className }: ChartProps) {
  ...
  return (
    <div className={cn("flex items-end gap-2 rounded-md bg-[var(--codex-surface-soft)] p-4", className)} style={{ height }}>
      {data.map((row, index) => {
        ...
        return (
          <div key={`${row[xField]}-${index}`} className="flex min-w-0 flex-1 flex-col items-center gap-2">
            ...
            <span className="max-w-full truncate text-[11px] text-codex-muted">{String(row[xField] ?? "")}</span>
          </div>
        );
      })}
    </div>
...
export function Bar({ data = [], xField = "value", yField = "label", height = 280, className }: ChartProps) {
  ...
  return (
    <div className={cn("space-y-3 rounded-md bg-[var(--codex-surface-soft)] p-4", className)} style={{ minHeight: height }}>
      {data.map((row, index) => {
        ...
        return (
          <div key={`${row[yField]}-${index}`} className="grid grid-cols-[minmax(80px,160px)_1fr_56px] items-center gap-3 text-xs">
            <span className="truncate text-codex-muted">{String(row[yField] ?? "")}</span>
            <span className="h-3 overflow-hidden rounded-full bg-[var(--codex-border)]">
              <span className="block h-full rounded-full" style={{ width: `${(value / max) * 100}%`, backgroundColor: colors[index % colors.length] }} />
            </span>
            <span className="text-right font-medium text-codex-ink">{value}</span>
          </div>
        );
      })}
    </div>
...
export function Line({ data = [], xField = "date", yField = "value", colorField = "metric", height = 280, className }: ChartProps & { point?: boolean; smooth?: boolean; legend?: unknown; axis?: unknown }) {
  ...
  return (
    <div className={cn("overflow-auto rounded-md bg-[var(--codex-surface-soft)] p-4", className)} style={{ height }}>
      ...
      <div className="mt-2 flex flex-wrap gap-3 text-xs text-codex-muted">
...
export function Pie({ data = [], angleField = "value", colorField = "type", height = 280, className }: ChartProps & { innerRadius?: number; legend?: unknown; label?: unknown }) {
  ...
  return (
    <div className={cn("grid items-center gap-4 rounded-md bg-[var(--codex-surface-soft)] p-4 sm:grid-cols-[160px_1fr]", className)} style={{ minHeight: height }}>
      <div className="relative mx-auto h-36 w-36 rounded-full" style={{
        ...
        <div className="absolute inset-10 rounded-full bg-[var(--codex-surface-soft)]" />
      </div>
      <div className="space-y-2 text-sm">
        {data.map((row, index) => (
          <div key={`${row[colorField]}-${index}`} className="flex items-center justify-between gap-3">
            <span className="inline-flex min-w-0 items-center gap-2 truncate text-codex-muted">
