...
export function FavoritePlanPanel({
  ...
  return (
    <section aria-label={t("create.favorite.ariaLabel")} className={`${embedded ? "" : "mt-3"} rounded-2xl border border-slate-100 bg-slate-50 p-3`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h3 className="inline-flex items-center gap-1.5 text-xs font-black text-slate-800">
            <Bookmark aria-hidden="true" className="h-3.5 w-3.5 text-[var(--codex-accent)]" /> {t("create.favorite.ariaLabel")}
          </h3>
          <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-slate-400">
            {t("create.favorite.description")}
          </p>
        </div>
        {!embedded && onToggleList ? (
          ...
          >
            {t("create.favorite.setsCount", { count: plans.length })}
      ...
      <div className="mt-3 flex gap-2">
        <label className="min-w-0 flex-1">
          <span className="sr-only">{t("create.favorite.nameLabel")}</span>
          <input
        ...
        >
          ...
          {isSaving ? t("create.favorite.saving") : t("create.favorite.save")}
        </button>
      </div>
              ...
              <div className="flex items-start gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-[11px] font-bold text-slate-400">
                    {formatSavedPlanMeta(plan, t)} · {formatSavedPlanTime(plan.updatedAt, t)}
                  </p>
                </div>
                <button
                  type="button"
                ...
                >
                  {t("create.favorite.apply")}
                </button>
                <button
                  type="button"
                  aria-label={t("create.favorite.deleteAria", { name: plan.name })}
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {plan.planPreview.slice(0, 4).map((module, index) => (
                    <span key={`${plan.id}-${module.name}-${index}`} className="rounded-full bg-slate-50 px-2 py-0.5 text-[10px] font-bold text-slate-500">
                      {module.usesModel ? `${t("create.favorite.modelPrefix")} ` : ""}{module.name}
                    </span>
                  ))}
...
type ProductSetTranslator = (key: string, values?: Record<string, string | number | Date>) => string;

function formatSavedPlanMeta(plan: SavedProductSetPlan, t: ProductSetTranslator) {
  const modeLabel = plan.mode === "custom" ? t("create.favorite.planCustom") : t("create.favorite.planVisual");
  const imageTypeLabel = plan.imageType === "details" ? t("create.favorite.typeDetails") : t("create.favorite.typeMain");
  const unit = plan.imageType === "details" ? t("units.singleScreen") : t("units.singleImage");
  const count = plan.planPreview.length || plan.genCount;
  return `${modeLabel} · ${imageTypeLabel} · ${count}${unit}`;
}

function formatSavedPlanTime(value: string, t: ProductSetTranslator) {
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? t("create.favorite.justNow") : savedPlanDateFormatter.format(date);
}
