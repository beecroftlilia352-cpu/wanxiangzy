...
function isAiConfigReady(config: AiConfig, model: string) {
    return Boolean(modelOptionName(model || config.model).trim());
}
            ...
            merge: (persisted, current) => {
                ...
                return {
                    ...
                    config: {
                        ...
                        videoModel: normalizeModelOptionValue(config.videoModel || defaultConfig.videoModel, channels) || defaultConfig.videoModel,
...
function normalizeChannels(config: AiConfig) {
    // Remove the retired built-in aggregation channel from existing browser
    // storage. Custom user-defined channels remain untouched.
    const hadRetiredPlatformChannel = Array.isArray(config.channels)
        && config.channels.some((channel) => channel.id === "platform");
    const persistedChannels = Array.isArray(config.channels)
        ? config.channels.filter((channel) => channel.id !== "platform")
        : [];
    ...
    if (!channels.length) {
        const retireLegacyDefaults = hadRetiredPlatformChannel;
        channels.push(
            createModelChannel({
                id: "default",
                name: "默认渠道",
                baseUrl: retireLegacyDefaults ? defaultConfig.baseUrl : config.baseUrl || defaultConfig.baseUrl,
                apiKey: config.apiKey || "",
                apiFormat: config.apiFormat || defaultConfig.apiFormat,
                models: retireLegacyDefaults ? [] : uniqueRawModels([
                    ...(config.models || []),
                    config.model,
                    config.imageModel,
                    config.videoModel,
