...
export function buildPoseGarmentAngleReferencePrompt(params: {
  ...
  return [
    "服装角度参考规则：",
    ...lines,
    "这些角度图不是新服装、不是局部纹理增强、不是人物/脸/姿势/身体/背景/光线参考；不要复制角度图中的模特、人体、姿势、裁切、场景或拍摄光线。",
    "生成姿势裂变时，主图 image 1 仍控制人物身份、原始穿搭关系、整体配色和可见服装外观；角度图只在同一件服装的隐藏面或转身可见面上补全结构。",
    "正面姿势不要强行放入背面元素，背面/侧面姿势也不要凭空改款；上下装冲突时，上装角度只影响上装，下装角度只影响下装，整套/连体角度只保持整体搭配关系。",
  ].join("\n");
}
