...
export const MATERIAL_ENHANCEMENT_LEVELS: Array<{
  ...
  },
  {
    ...
    label: "商业精修",
    description: "干净高级成片",
    prompt: "增强强度：商业精修。让服装材质更干净、清晰、可信，统一服装区域的曝光和微对比，保持高级商品图质感，不改变人物、姿势和背景。",
  },
];
...
export function buildMaterialEnhancementPrompt(params: {
  ...
  return [
    ...
    `任务：对图1中可见的${garmentType}区域做商用级材质增强，提升服装细节质感，让成片更适合电商主图、详情页和内容投放。`,
    ...
    level.prompt,
    "材质表现：增强应像专业商业修图和高清商品摄影，不是重新生成一件衣服；保留服装固有色、图案位置、logo 形状和新旧状态，避免全图锐化、HDR、磨皮、过度微对比、滤镜化或塑料感。",
    "输出质量：photorealistic, 8K ultra-detailed, RAW photo quality, true-to-source garment material enhancement, commercial product retouching, neutral commercial color management.",
    `用户补充：${userPrompt || "无，按商用材质增强最佳实践执行。"}`,
    "负面约束：不要换脸，不要换人，不要改身体，不要换背景，不要换服装款式，不要改服装主色，不要改图案/logo位置，不要新增不存在的口袋/纽扣/拉链，不要把服装重绘成另一件商品，不要油画感、插画感、AI渲染感、水印或文字。",
  ].join("\n");
}
...
export function enforceMaterialEnhancementPromptRequirements(prompt: string, params: {
  ...
  if (normalized.includes("图像角色：图1是最终画面原图") && normalized.includes("编辑边界")) return normalized;
  return `${base}\n\n补充执行要求：${normalized}`;
}
