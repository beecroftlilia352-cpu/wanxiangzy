...
export const WORKFLOW_TOOLS: Record<WorkflowToolType, ToolDefinition> = {
  ...
  commerce_detail_section: {
    ...
    enabled: true,
    title: "详情页板块",
    description: "为淘宝、天猫、京东、PDD、抖音、小红书、独立站等平台生成单个手机详情页板块，适合逐张生成后拼接成长图。",
    inputSchema: { referenceImages: "image[]?", prompt: "string", sectionTitle: "string", platform: "string" },
    outputSchema: { imageUrls: "string[]" },
    requiredCapabilities: ["text_to_image"],
    costPolicy: { baseCredits: 1, perImage: true },
    retryPolicy: DEFAULT_RETRY,
  ...
  commerce_detail_stitch: {
    ...
    enabled: true,
    title: "详情页长图拼接",
    description: "把多个详情页板块按手机端宽度拼接成长条预览图，同时保留独立板块。",
    inputSchema: { imageUrls: "string[]", width: "number", gap: "number" },
    outputSchema: { imageUrls: "string[]" },
