...
export function buildTryOnFacePrompt(params: TryOnPromptContext = {}) {
  ...
  if (params.hasReference) {
    return `模特脸规则（身份迁移）：${faceRef}是最终脸部身份锚点，控制可识别身份、脸型轮廓、眼睛形状和间距、眉形、鼻梁/鼻尖/鼻翼结构、嘴部固有形状、五官大小比例和整体相似度；${referenceRef}只作为自然表情、肤色、妆容、头部姿态、头部大小、光影和皮肤连续性的驱动图。必须把${referenceRef}的表情方向、表情强度、肤色和妆容自然迁移到${faceRef}身份上，而不是为了保留表情而保留${referenceRef}原脸。${referenceRef}原来的眼睛、鼻子、嘴巴和脸型不能保留为最终身份特征，但${referenceRef}的表情线索要作为自然表演依据：嘴巴开合、嘴角方向、笑/不笑强度、眼睛睁合、眉毛紧张度、视线、下颌放松程度和整体情绪都参考${referenceRef}，允许为了让${faceRef}本人看起来自然而做轻微人脸微调，避免面具感和证件照感。最终脸必须一眼看出来自${faceRef}本人；如果不像${faceRef}，即使服装、姿势或表情正确也算失败。表情处理方式是让${faceRef}这个人自然做出${referenceRef}的表情状态：${referenceRef}冷脸闭嘴时，最终是${faceRef}身份的自然冷脸闭嘴；${referenceRef}微笑时，最终是${faceRef}身份按${referenceRef}强度自然微笑。肤色处理采用参考图原则：按${referenceRef}的肤色、妆容和场景光线重新打光，使脸、颈部、胸口、手臂、手部等可见皮肤像同一张照片里连续拍摄，保留毛孔、轻微瑕疵、局部红润、真实阴影和自然不对称。${faceRef}只在身份/相似度/五官结构上优先，${referenceRef}在自然表情/肤色/妆容/姿态/比例/光影上优先；不要证件照式正脸、贴上去的头、面具边缘、不同图层光影、头过大/过小、长脖子、肤色断层、过度磨皮、小V脸、雪白皮、假笑模板脸、复制${faceRef}笑容、复制${faceRef}肤色、复制${faceRef}妆容、保留${referenceRef}原脸身份或生成弱相似度的通用网红脸。最终年龄感必须与${targetIdentity}身体自然匹配。`;
  }
