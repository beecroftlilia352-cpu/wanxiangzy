export const TRYON_QUALITY =
  "photorealistic natural camera photo, reference-matched fashion photo quality, realistic skin texture, true-to-source fabric texture at visible scale, believable light and shadow, unforced natural expression, non-synthetic real-person appearance, no extra sharpening, no HDR, no moire or wavy fabric artifacts";
...
export const TRYON_PHOTOGRAPHY_RULE =
  "商业摄影规则：画面必须像真实相机拍摄的服装图片，肤色自然、毛孔和轻微瑕疵可见、表情不过度营业、布料纹理和背景材质真实、光影方向可信、景深和焦点自然；允许真实照片里的轻微不完美和非对称感。不要生成过干净棚拍、素材库模板表情、AI精修脸、蜡像、塑料皮肤或过度磨皮。";
...
export const TRYON_FACE_RULE =
  "模特脸规则：模特脸图是最终脸部身份锚点，提供可识别身份、完整五官结构、五官大小比例、脸型轮廓和五官相对位置；不提供肤色、妆容、表情、身体比例、年龄身高、头部大小、肩宽、四肢长度、服装、姿势、背景、构图或场景光线。有参考人物时，必须把参考图的整体表情表演、肤色、妆容和光影迁移到模特脸身份上，而不是保留参考图原脸；最终脸必须仍然一眼像模特脸图本人。不要复制模特脸图的原始表情、雪白肤色或妆容，不要保留参考图原脸身份，不要证件照式换脸、大头、长脖子、肤色断层、不同图层光影、过度磨皮、小V脸、模板表情脸或过度对称的AI脸。";
...
export function buildTryOnFacePrompt(params: TryOnPromptContext = {}) {
  ...
  if (params.hasReference) {
    return `模特脸规则（身份迁移）：${faceRef}是最终脸部身份锚点，控制可识别身份、脸型轮廓、眼睛形状和间距、眉形、鼻梁/鼻尖/鼻翼结构、嘴部固有形状、五官大小比例和整体相似度；${referenceRef}只作为自然表情、肤色、妆容、头部姿态、头部大小、光影和皮肤连续性的驱动图。必须把${referenceRef}的整体表情表演状态、表情类别、强度、情绪方向、视线行为、面部张力和自然不对称迁移到${faceRef}身份上，而不是为了保留表情而保留${referenceRef}原脸。${referenceRef}原来的眼睛、鼻子、嘴巴和脸型不能保留为最终身份特征，但${referenceRef}的表情状态要作为完整表演依据；允许为了让${faceRef}本人看起来自然而做轻微人脸微调，避免面具感和证件照感，但自然融合只允许调整表情肌肉、视线、肤色重打光、妆容匹配、毛孔、阴影和边缘融合；不要为了自然而改动${faceRef}的脸型轮廓、眼睛形状、眼距、眉形、鼻子结构、嘴部结构、五官比例或身份相似度。最终脸必须一眼看出来自${faceRef}本人；如果不像${faceRef}，即使服装、姿势或表情正确也算失败。禁止素材库假表情和模板脸，但不能因此压平、移除或重设计${referenceRef}中真实存在的自然表情。肤色处理采用参考图原则：按${referenceRef}的肤色、妆容和场景光线重新打光，使脸、颈部、胸口、手臂、手部等可见皮肤像同一张照片里连续拍摄，保留毛孔、轻微瑕疵、局部红润、真实阴影和自然不对称。${faceRef}只在身份/相似度/五官结构上优先，${referenceRef}在自然表情/肤色/妆容/姿态/比例/光影上优先；不要证件照式正脸、贴上去的头、面具边缘、不同图层光影、头过大/过小、长脖子、肤色断层、过度磨皮、小V脸、雪白皮、模板表情脸、复制${faceRef}原表情、复制${faceRef}肤色、复制${faceRef}妆容、保留${referenceRef}原脸身份或生成弱相似度的通用网红脸。最终年龄感必须与${targetIdentity}身体自然匹配。`;
  }

  return `模特脸规则：${faceRef}是唯一脸部身份参考，最终人物脸部身份必须使用该图的五官、五官大小比例、脸型倾向和身份气质；${faceRef}不提供身体比例、服装、姿势、背景、构图、固定表情、固定肤色或固定妆容。最终头部大小、脖颈长度、肩颈连接、身体肤色和光影要和生成身体统一，年龄感必须与${targetIdentity}身体自然匹配；表情保持自然克制，不要复制夸张模板表情。不要把服装图或参考图中的脸误当成最终身份，不要自动改成标准鹅蛋脸、小V脸、雪白皮、模板表情脸、素材库表情脸或过度对称的AI脸。`;
}
...
export function buildTryOnNegativePrompt(params: TryOnPromptContext = {}) {
  ...
  const intimateNegative = garmentCategory === "intimate"
    ...
    : "";
  const base = "负面约束：不要生成多余人物，不要扭曲身体和服装，不要改变服装结构，不要保留错误的参考图原脸，不要硬贴脸，不要面具边缘，不要肤色断层，不要自动美白，不要雪白皮或冷白皮，不要塑料皮肤，不要蜡像感，不要卡通感，不要AI渲染感，不要素材库模板表情，不要过干净灰底棚拍，不要过度对称AI脸，不要文字水印";
