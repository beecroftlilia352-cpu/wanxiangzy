...
const KIND_HEADERS: Record<ImagePromptKind, string> = {
  ...
  outfitFusion:
    "核心任务：按用户输入框内的可见关系句完成搭配融图，不追加隐藏模板约束。",
  grass:
...
const REQUIRED_SIGNALS: Record<ImagePromptKind, RequiredSignal[]> = {
  ...
  modelBackground: [
    ...
    { name: "画幅裁切", pattern: /画幅与裁切锁定|景别|半身|扩图成全身|镜头距离/, fallback: "画幅与裁切锁定：最终输出必须保持图1原始画幅比例、镜头距离、景别、主体大小和可见身体范围；半身保持半身，不要拉远镜头或扩图成全身。" },
