...
describe("compileImagePromptForModel", () => {
  ...
  it("passes outfit fusion prompts through without adding backend constraints", () => {
    ...
    expect(result).not.toContain("负面约束");
    expect(result).not.toContain("脸部硬规则");
    expect(result).not.toContain("商品保真");
  });
