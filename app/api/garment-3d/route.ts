...
function buildServerPrompt(params: {
  ...
  const referenceLine = params.hasReference
    ? "参考图2的服装立体感、袖身厚度、阴影结构、空间角度和棚拍质感，但不要复制图2的颜色、图案、文字或具体款式。"
    : "根据用户提示生成类似穿在人身上的立体效果。";

  return `${roles} 任务：将图1的${params.garmentType}从平面图或人台图转换为无真人、无头部、无脸、无手的3D立体服装展示图。${referenceLine} 严格保留图1服装的版型、颜色、材质、纹理、图案、纽扣、拉链、口袋、帽绳、袖口、裤腰、裤脚等细节。保持原背景风格不变或尽量接近原背景，主体居中，边缘干净，真实商业棚拍质感。${params.userPrompt.trim()} 负面约束：不要生成真人身体、不要生成模特脸、不要多件衣服、不要改变衣服品类、不要扭曲文字和 logo、不要改变主要颜色。`;
}
