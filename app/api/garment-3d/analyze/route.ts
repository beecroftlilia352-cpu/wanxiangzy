...
export async function POST(request: NextRequest) {
  try {
    ...
    const textPrompt = `你是电商服装图像生成提示词工程师。请分析�?中的服装，判断它是正面、背面、平铺图还是人台图，并生成一段用于“服装转3D”的中文提示词�?
图像角色：图1是用户上传的${finalType}�?目标：把�?服装变成类似穿在人身上的3D立体服装展示效果，但不要出现真人、头部、脸、手或人体皮肤。根据图1判断正面或背面：如果是正面，就做正面微微向左旋转的立体效果；如果是背面，就保留背面视角并微微向左旋转；如果是平铺图，要让衣服自然撑起，有厚度、袖身体积和真实褶皱�?必须保留：服装品类、版型、颜色、材质、图案、文�?logo位置、纽扣、拉链、口袋、帽子、袖口、裤腰、裤脚和破洞/水洗/纹理细节�?画面要求：主体居中，商业棚拍，柔和阴影，背景尽量保持�?背景不变，边缘干净，真实布料体积感�?负面约束：不要生成真人身体，不要生成模特脸，不要多件衣服，不要改变服装类型，不要改变主色，不要扭曲文字和 logo，不要卡通感�?
用户当前提示词：
${prompt || ""}

只输出最终中文提示词�?0-180字，不要解释。`;
