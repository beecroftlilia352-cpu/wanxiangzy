...
export async function PATCH(request: Request, { params }: RouteProps) {
  ...
  if (current.config_key === "model.routing" || current.config_key === "model.providers") {
    return NextResponse.json({ error: `${current.config_key} 已废弃，只能查看历史记录，不能再次发布或归档` }, { status: 409 });
  }
