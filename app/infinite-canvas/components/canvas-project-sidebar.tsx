            ...
            <aside className="hidden w-14 shrink-0 border-r border-stone-200 bg-white md:flex md:flex-col md:items-center md:gap-3 md:py-4">
                ...
                {projects.slice(0, 8).map((project) => (
                    <button
                        ...
                        className={cn(
                            "size-9 overflow-hidden rounded-md border border-stone-200 bg-stone-50",
    ...
    return (
        <aside className="hidden w-64 shrink-0 flex-col border-r border-stone-200 bg-white md:flex">
            <div className="flex items-center justify-between border-b border-stone-200 px-4 py-3">
                ...
                <button type="button" onClick={toggleSidebar} className="text-xs font-medium text-stone-500 hover:text-stone-900" aria-label="收起侧栏">
                    收起
                </button>
            </div>
            <div className="space-y-3 px-3 py-3">
                <div className="relative">
                    ...
                    <Input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="搜索画布" className="h-9 pl-8 text-sm" />
                </div>
                <div className="flex items-center gap-1.5">
                    <FilterChip active={!starredOnly} onClick={() => setStarredOnly(false)}>
                        全部
                    </FilterChip>
                    <FilterChip active={starredOnly} onClick={() => setStarredOnly(true)} icon={<Star className={cn("size-3", starredOnly && "fill-current")} />}>
                        收藏
                    </FilterChip>
                </div>
            </div>
                        ...
                        {filtered.map((project) => {
                            ...
                            return (
                                <li key={project.id}>
                                    <div
                                        className={cn(
                                            "group flex cursor-pointer items-center gap-2.5 rounded-md px-2 py-1.5 transition",
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                "grid size-7 shrink-0 place-items-center rounded text-stone-400 transition hover:bg-white hover:text-stone-700",
