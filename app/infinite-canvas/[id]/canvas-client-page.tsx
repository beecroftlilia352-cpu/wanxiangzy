...
function CanvasRefreshShell() {
    return (
        <main className="relative h-screen min-h-0 overflow-hidden bg-background text-foreground">
            <div
...
function InfiniteCanvasPage() {
    ...
    const getConnectionDropTarget = useCallback(
        (clientX: number, clientY: number, current: ConnectionHandle): ConnectionDropTarget => {
            ...
            for (let index = currentNodes.length - 1; index >= 0; index -= 1) {
                ...
                isNearNode = true;
                if (node.id === current.nodeId || !normalizeConnection(current.nodeId, node.id, currentNodes, current.handleType)) continue;
    ...
    const configInputsById = useMemo(() => {
        if (nodeDraggingRef.current && configInputsByIdRef.current.size) return configInputsByIdRef.current;
        const map = new Map<string, NodeGenerationInput[]>();
        nodes.forEach((node) => {
            if (node.type !== CanvasNodeType.Config) return;
            map.set(node.id, buildNodeGenerationInputs(node.id, nodes, connections));
        });
        configInputsByIdRef.current = map;
        return map;
    }, [connections, nodes]);
    const resourceContextNodeId = dialogNodeId || activeNodeId;
    const canvasResourceReferences = useMemo(() => {
        if (nodeDraggingRef.current) return canvasResourceReferencesRef.current;
        const references = buildCanvasResourceReferences(nodes, connections, resourceContextNodeId);
        canvasResourceReferencesRef.current = references;
        return references;
    }, [connections, nodes, resourceContextNodeId]);
    const resourceReferenceByNodeId = useMemo(() => new Map(canvasResourceReferences.map((reference) => [reference.nodeId, reference])), [canvasResourceReferences]);
    const mentionReferencesByNodeId = useMemo(() => {
        if (nodeDraggingRef.current && mentionReferencesByNodeIdRef.current.size) return mentionReferencesByNodeIdRef.current;
        const map = new Map<string, ReturnType<typeof buildNodeMentionReferences>>();
        nodes.forEach((node) => map.set(node.id, buildNodeMentionReferences(node, nodes, connections)));
        mentionReferencesByNodeIdRef.current = map;
        return map;
    }, [connections, nodes]);
    ...
    return (
        <main className="flex h-screen min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                <CanvasTopBar
                    ...
                    onHome={() => router.push("/")}
                    onProjects={() => router.push("/infinite-canvas")}
        ...
        <>
            <div className="pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-16 items-center justify-between px-4">
                <div className="pointer-events-auto flex min-w-0 items-center gap-3">
                    <div ref={menuRef} className="canvas-header-local-menu relative" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                        <button
                            type="button"
                        ...
                        {menuOpen ? (
                            <div
                                data-canvas-top-menu
                                className="absolute left-0 top-[calc(100%+8px)] z-[80] w-[220px] rounded-xl border p-1 text-sm shadow-2xl"
...
function CanvasMenuButton({ icon, label, danger = false, disabled = false, onClick }: { icon: ReactNode; label: ReactNode; danger?: boolean; disabled?: boolean; onClick: () => void }) {
    return (
        <button
            type="button"
            role="menuitem"
            disabled={disabled}
            className={`flex h-9 w-full items-center gap-2 rounded-lg px-2.5 text-left transition hover:bg-black/5 disabled:cursor-not-allowed disabled:opacity-35 dark:hover:bg-white/10 ${danger ? "text-red-500" : ""}`}
            onClick={onClick}
        >
...
function CanvasMenuDivider() {
    return <div className="my-1 h-px bg-current opacity-10" role="separator" />;
}
